...
// манипулировать из разных мест
    ...
    private JMenuItem saveToTable;
    private JMenuItem searchValueMenuItem;
    private JMenuItem showAboutProgramItem;
...
// Добавить его в главное меню
        ...
        JMenu tableSprav = new JMenu("Справка");
// Добавить его в главное меню
        menuBar.add(tableSprav);
...
// По умолчанию пункт меню является недоступным (данных ещѐ нет)
        ...
        // Создать новое "действие" по сохранению в текстовый файл
        Action actionSaveToTable = new AbstractAction("Сохранить данные для построение таблицы") {
            public void actionPerformed(ActionEvent event) {
                if (fileChooser==null) {
// Если экземпляр диалогового окна
// "Открыть файл" ещѐ не создан,
...
// Если результат его показа успешный,
// сохранить данные в двоичный файл
                saveToTableFile(fileChooser.getSelectedFile());//save to table
            }
        };
// Добавить соответствующий пункт подменю в меню "Файл"
        saveToTable = fileMenu.add(actionSaveToTable);
// По умолчанию пункт меню является недоступным (данных ещѐ нет)
        saveToTable.setEnabled(false);
...
// Установить введенное значение в качестве иголки
                ...
                // Задаем,что нам нужно именно найти значения
                renderer.setType("FindingNum");
// Обновить таблицу
...
// По умолчанию пункт меню является недоступным (данных ещѐ нет)
        ...
        Action showPrimeNumber = new AbstractAction("Показать близкие к простым числа") {
            public void actionPerformed(ActionEvent event) {
                // Задаем,что нам нужно именно найти простые числа
                renderer.setType("FindingPrimeNum");
// Обновить таблицу
                getContentPane().repaint();
            }
        };
        showPrimeNumber.setEnabled(false);
        tableMenu.add(showPrimeNumber);

        Action showAboutProgram = new AbstractAction("О программе") {
            public void actionPerformed(ActionEvent event) {
                AboutProgram aboutProgram = new AboutProgram();
                aboutProgram.setVisible(true);
            }
        };
        showAboutProgramItem = tableSprav.add(showAboutProgram);
...
// Считать значения начала и конца отрезка, шага
                    ...
                    Double step = Double.parseDouble(textFieldStep.getText()); if(step == 0) throw new Exception("Шаг не может быть нулем");
// На основе считанных данных создать новы экземпляр модели таблицы
...
// Пометить ряд элементов меню как доступных
                    ...
                    showPrimeNumber.setEnabled(true);
                    saveToTable.setEnabled(true);
...
// В случае ошибки преобразования чисел показать сообщение об ошибке
                ...
                } catch(Exception ex){JOptionPane.showMessageDialog(MainFrame.this, ex.getMessage(), "Ошибочный формат числа", JOptionPane.WARNING_MESSAGE);}
            }
        });
...
// Записать в поток вывода значения в точках
            for (int i = 0; i<data.getRowCount(); i++) {
                out.println("Значение в точке " + data.getValueAt(i,0)
                        + " по Схеме Горнера равно " + data.getValueAt(i,1)
                        + ", как многочлен равно " + data.getValueAt(i,2)
                        + ", разница " + data.getValueAt(i,3));
            }
// Закрыть поток
            out.close();
...
// обрабатывать, так как мы файл создаѐм, а не открываем
    ...
    protected void saveToTableFile(File selectedFile) {
        try {
// Создать новый поток вывода для таблицы, направленный в указанный файл
            PrintWriter out = new PrintWriter(new
                    FileOutputStream(selectedFile));
// Записать в поток вывода попарно значение X в точке, значение многочлена в точке
            for (int i = 0; i < data.getRowCount(); i++) {
                out.print((Double)data.getValueAt(i,0));
                out.write(',');
                out.print((Double)data.getValueAt(i,1));
                out.write(',');
                out.print((Double)data.getValueAt(i,2));
                out.write(',');
                out.print((Double)data.getValueAt(i,3));
                out.write('\n'); 
            }
// Закрыть поток вывода
            out.close();
